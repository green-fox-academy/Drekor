...
GPIO_InitTypeDef tdf10;
GPIO_InitTypeDef tdf9;
...
int main(void) {
	...
	tdf10.Pin = GPIO_PIN_10;
	tdf10.Mode = GPIO_MODE_OUTPUT_PP;
	tdf10.Pull = GPIO_PULLDOWN;
	tdf10.Speed = GPIO_SPEED_HIGH;


	tdf9.Pin = GPIO_PIN_9;
	tdf9.Mode = GPIO_MODE_OUTPUT_PP;
	tdf9.Pull = GPIO_PULLDOWN;
	tdf9.Speed = GPIO_SPEED_HIGH;

	HAL_GPIO_Init(GPIOA, &tda);
	HAL_GPIO_Init(GPIOF, &tdf10);
	HAL_GPIO_Init(GPIOF, &tdf9);


	TimHandle.Instance = TIM2;
	TimHandle.Init.Period = 50000;
	TimHandle.Init.Prescaler = 32400;
	...
	while (1) {
		...
		int timer = __HAL_TIM_GET_COUNTER(&TimHandle);
