...
int main(void) {
	...
	GPIO_InitTypeDef kulso;            // create a config structure
		kulso.Pin = GPIO_PIN_8;            // this is about PIN 0
		kulso.Mode = GPIO_MODE_OUTPUT_PP; // Configure as output with push-up-down enabled
		kulso.Pull = GPIO_PULLDOWN;      // the push-up-down should work as pulldown
		kulso.Speed = GPIO_SPEED_HIGH;     // we need a high-speed output

		HAL_GPIO_Init(GPIOF, &kulso);   // initialize the pin on GPIOA port with HAL
	...
	//pink//		HAL_GPIO_WritePin(GPIOF, GPIO_PIN_9, GPIO_PIN_RESET); // setting the pin to 0 //red

				//	HAL_GPIO_WritePin(GPIOF, GPIO_PIN_9, GPIO_PIN_SET); // setting the pin to 1 //red

				//	HAL_GPIO_WritePin(GPIOF, GPIO_PIN_10, GPIO_PIN_RESET); // setting the pin to 0 //green

	//pink//		HAL_GPIO_WritePin(GPIOF, GPIO_PIN_10, GPIO_PIN_SET); // setting the pin to 1 //green
